...
module.exports = {
  ...
  hyphenate(str) {
    return str.includes('-') ? str : str.replace(/([A-Z])/g, "-$1")
  },
  /**
  ...
  /**
   * Get the attribute which has the given pascal name.
   * @param {ASTNode} node The start tag node to check.
   * @param {string} name The attribute name to check.
   * @param {string} [value] The attribute value to check.
   * @returns {ASTNode} The found attribute.
   */
  getAttributeByPascal(node, name, value) {
    assert(node && node.type === 'VElement')
    return node.startTag.attributes.find(a =>
      !a.directive &&
      this.hyphenate(a.key.name) === name &&
      (
        value === undefined ||
        (a.value != null && a.value.value === value)
      )
  ...
  /**
   * Get the directive which has the given pascal name.
   * @param {ASTNode} node The start tag node to check.
   * @param {string} name The directive name to check.
   * @param {string} [argument] The directive argument to check.
   * @returns {ASTNode} The found directive.
   */
  getDirectiveByPascal(node, name, argument) {
    assert(node && node.type === 'VElement')
    return node.startTag.attributes.find(a =>
      a.directive &&
      this.hyphenate(a.key.name.name) === name &&
      (argument === undefined || (a.key.argument && this.hyphenate(a.key.argument.name)) === argument)
    )
  },
  /**
