    ...
    return utils.defineTemplateBodyVisitor(
      context,
      {
        VElement(node) {
            ...
            for (let option of option2.attribute) {
              ...
              if (!attr && !dir) {
                if (option.defaultText) {
                  ...
                  reportItem.fixadd = `${option.name}="${option.defaultText}"`
                  ...
                  reportItem.fixadd = option.mustBeDirective ? `v-model:${option.name}="${option.defaultValue}"` : `:${option.name}="${option.defaultValue}"`
                } else {
          ...
          function fix(fixer) {
            let startToken = tokens.getFirstToken(EleNode)
            let res = []
            reports.forEach((item) => { 
              if (item.fixadd) {
                item.fixadd =' ' + item.fixadd
                res.push(fixer.insertTextAfter(startToken,item.fixadd)) 
              }
            })
            return res
          }
